...
public class Repository {
    ...
    public static void init() {
        ...
        initHeads(initialCommit); //Heads文件夹存放的是分支文件，文件内容是该分支最新的commit的哈希值id(文件名是分支名，文件内容是commitID)
    }
    ...
    public static void log() {
        for (lastCommit = readLastCommit();
             hasParentCommit(lastCommit); moveLastToPreviousCommit()) {
        ...
        for (String committedFile : committedFiles.keySet()) {
            ...
            if (cwdFiles.contains(relativePath)) {
                ...
                if (!Arrays.equals(blobContent, cwdContent)
                        && !addStageFiles.containsKey(committedFile)) {
    ...
    private static void printUntrackedFiles() {
        ...
        for (String cwdFile : cwdFiles) {
            ...
            if (!committedFiles.containsKey(absolutePath)
                    && !addStageFiles.containsKey(absolutePath)
    ...
    public static void merge(String targetBranch) {
        ...
        Commit targetCommit = readObject(
                join(OBJECTS_DIR, targetBranchCommitId), Commit.class);
        ...
        Map<String, String> overwriteMap =
                overWriteFiles(splitPointMap, curCommitMap, targetCommitMap);
        ...
        Map<String, String> conflictMap = calculateConflict(
                allFiles, splitPointMap, curCommitMap, targetCommitMap);  //处理冲突的情况
    ...
    private static void checkMerge(Map<String, String> writeMap,
        ...
        for (String cwdFile : absoluteCwdFiles) {
            if (!curCommitMap.containsKey(cwdFile)) {
                if (writeMap.containsKey(cwdFile)
                        || removeMap.containsKey(cwdFile)) {
                    message("There is an untracked file in the way; "
                            + "delete it, or add and commit it first.");
                    System.exit(0);
                }
    ...
    private static Map<String, String> calculateConflict(List<String> allFiles,
    ...
    private static void writeConflictFile(String path, String curBranchContent,
                                          String assignedBranchContent) {
    ...
    private static Map<String, String> removeMap(Map<String, String> splitPointMap,
                                                 Map<String, String> curCommitMap,
                                                 Map<String, String> targetCommitMap) {
        ...
        for (String splitFilepath : splitPointMap.keySet()) {
            if (curCommitMap.containsKey(splitFilepath)
                    && !targetCommitMap.containsKey(splitFilepath)) {
    ...
    private static Map<String, String> writeMap(Map<String, String> splitPointMap,
                                                Map<String, String> curCommitMap,
                                                Map<String, String> targetCommitMap) {
        Map<String, String> writeMap = new HashMap<>();
        //只在给定分支中存在，在分割点和当前分支都不存在
        for (String targetFilePath : targetCommitMap.keySet()) {
            if (!splitPointMap.containsKey(targetFilePath)
                    && !curCommitMap.containsKey(targetFilePath)) {
    ...
    private static Map<String, String> overWriteFiles(Map<String, String> splitPointMap,
        ...
        for (String path : splitPointMap.keySet()) {
            if (curCommitMap.containsKey(path) && targetCommitMap.containsKey(path)) {
                ...
                if (splitPointBlobId.equals(curCommitBlobId)
                        && !splitPointBlobId.equals(targetCommitBlobId)) {
    ...
    private static List<String> allFilesList(Commit splitPoint, Commit curCommit,
                                             Commit targetCommit) {
